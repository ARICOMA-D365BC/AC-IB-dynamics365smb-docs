...
The following lines describe settings from those necessary for the actual commissioning (Application Registration in Entra, SharePoint Entra settings) to settings covering special customer requirements (e.g. when storing files or recording additional attributes).
...
|-|-|
| DOCLINKS_USAGE_ACH | For general use|
| DOCLINKS_SETUP_ACH | For configuring the module’s behavior |

## Registering an application in Entra
