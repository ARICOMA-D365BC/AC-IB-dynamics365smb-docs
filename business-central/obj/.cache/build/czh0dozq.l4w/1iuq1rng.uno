...
<ol>
<li><p>Choose the <img src="media/ui-search/search_small.png" alt="Lightbulb that opens the Tell Me feature." title="Tell me what you want to do"> icon, enter <strong>Sales Invoices</strong> and then choose the related link.</p>
</li>
<li><p>On the Sales Invoices page, click <em>New</em> to create a new sales invoice.</p>
</li>
<li><p>Complete the lines of the document as needed.</p>
<div class="IMPORTANT">
<h5>Important</h5>
...
<li><p>In the <strong>Payment Method Code</strong> field, select the payment method linked to a fiscal printer (see <a href="sk-fiscalprinters-integration-setup.html#payment-methods-settings">Setting payment methods</a> with the <strong>Fiscal Tax Document</strong> flag enabled).</p>
</li>
<li><p>Post the document.</p>
</li>
</ol>
...
<p>This scenario describes cash payment of a posted invoice if the fiscal printer in Business Central is connected to the Bank Account entity. If connected to the Cash Desk entity, simply create a cash receipt in the usual way (similar to <a href="#payment-of-advance-invoice-on-the-fiscal-printer">Payment of advance invoice on the fiscal printer</a> below).</p>
<div class="NOTE">
<h5>Note</h5>
...
Additionally, it is possible to perform the corresponding operations in the Business Central with the fiscal printer switched off and the created fiscal ledger entries are additionally manually marked as “Registered” (see below).</p>
<h3 id="manual-receipt">Manual Receipt</h3>
<p>If a printer connection error occurs while posting a document, the document is posted and a fiscal item is created. This item must be registered once the fiscal printer is operational.
If a user creates a manual receipt, it must be registered under the manual document number along with the date and time of issuance. The <em>Register Entry</em> function is used for this purpose (see <a href="#registering-entries">Registering entries</a> below).</p>
...
<p>In cases where the user has issued a manual receipt (sales slip), the user must register the item retroactively:</p>
<ol>
...
<li>Enter the receipt number in the <strong>Receipt No.</strong> field.</li>
<li>Enter the date and time the receipt was issued in the <strong>Issued date and time</strong> field, then click OK.</li>
</ol>
<p>In cases where the user is able to resolve the issue (e.g., the printer was turned off), the user should register the item retroactively:</p>
<ol>
<li>Choose the <img src="media/ui-search/search_small.png" alt="Lightbulb that opens the Tell Me feature." title="Tell me what you want to do"> icon, enter <strong>Fiscal Ledger Entries</strong> and then choose the related link.</li>
<li>On the <strong>Fiscal Ledger Entries</strong> page, locate the appropriate record and run the <em>Register Entry</em> action.</li>
<li>Leave all fields blank and click OK.</li>
</ol>
<div class="NOTE">
<h5>Note</h5>
...
<p>In case of confusion, it is possible to view the files exchanged between the Business Central and the fiscal printer. These are recorded directly in the entries:</p>
<ol>
<li>Choose the <img src="media/ui-search/search_small.png" alt="Lightbulb that opens the Tell Me feature." title="Tell me what you want to do"> icon, enter <strong>Fiscal Ledger Entries</strong> and then choose the related link.</li>
